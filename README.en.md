...
**Source install only.** Use the GitHub-generated source archive from [`v0.1.2`](https://github.com/Jia-Ethan/zcode-keysmith/releases/tag/v0.1.2), or clone this repo and run `python3 zcode-keysmith.py`. There are currently no standalone binary assets or pip/npm package.
...
Documented support is macOS with a local `ZCode.app`, and Windows 10/11 with a local `ZCode.exe`. The `v0.1.2` Release provides only GitHub-generated source archives; desktop installers are not yet published. Python 3.10+ is recommended; Windows must retain the Python interpreter used during install.
